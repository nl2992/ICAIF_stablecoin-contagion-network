...
\begin{abstract}
Existing stablecoin stress analyses rely on price-based proxies that miss the
on-chain liquidity-flow layer: every AMM swap, mint, and burn is immutably
recorded in on-chain logs, yet this execution-grade evidence is rarely used
for causal inference.  We construct the first provenance-gated multi-layer
network of stablecoin stress propagation, using Curve Finance
\texttt{TokenExchange} logs as Tier-A evidence, and introduce a three-gate
pipeline (provenance $\rightarrow$ statistical $\rightarrow$ paper) that
prevents fixture or Tier-B data from silently reaching paper-level claims.
We make three contributions.  \emph{Methodologically}: (a)~the
provenance-aware evidence-gating pipeline; (b)~a claim-level taxonomy
distinguishing A/A DEX-flow, A/A settlement, A/B directional, and B/B
contextual evidence.  \emph{Empirically}: (a)~the first A/A lead-lag result
for \emph{within-AMM-layer} stablecoin stress propagation---Curve 3pool and
Curve crvUSD/USDT exhibit Bonferroni-significant bidirectional co-movement
($\hat{\rho}=0.386$, Bonferroni $p\le 0.014$, $n=168$ hourly observations)
during USDT/Curve 2023; (b)~a comparative cross-event analysis showing that
claim strength is highest for endogenous AMM stress and lowest for exogenous
shocks that originate outside the DEX layer.  A four-classifier ML benchmark
also achieves AUROC~$= 0.859$ (Random Forest) on USDC/SVB 2023;
Tier-A AMM-flow features contribute $+5.1$~pp over a Tier-B-only baseline.
\end{abstract}
