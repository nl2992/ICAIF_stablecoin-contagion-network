...
| Arbitrage flip — distribution-free null (corroborating) | label-shuffle p=0.002 USDT/Curve, 0.029 BUSD, **0.19 FTX (ns)** — only the supply/regulatory flips survive; parametric p overstated FTX | `results/tables/arbitrage_regime_permutation_null.json` (← `scripts/run_arbitrage_regime_permutation_null.py`) | `p_permutation_two_sided`; reproduces `fisher_z_obs` as a gate |
...
| TVP-VAR spillover concentrates post-onset (corroborating) | 91–100% of cross-edge FEVD post-onset across the 5 events (pooled 95%), near-zero pre-onset | `results/tables/tvp_var_post_onset_share.json` (← `scripts/run_tvp_var_post_onset_share.py`, recomputed from `table_tvp_var_spillovers_*.csv`) | `pooled_post_onset_share`=0.9552, `per_event_min`=0.913; corroborating robustness check, secondary to FR + HMM |
