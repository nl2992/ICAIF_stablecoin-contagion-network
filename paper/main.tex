...
A price-based DeFi stress monitor observes stablecoin basis widening after the
stress has already manifested in market prices.  By contrast, Curve Finance
\texttt{TokenExchange} logs record every swap with an exact block timestamp and
exact token amount---data that is freely available and updated every 12 seconds.
In the June~2023 USDT/Curve episode, abnormal net USDC selling in Curve pools
is observable in on-chain logs before the corresponding basis dislocation
appears in any CEX price feed.  Every existing stablecoin contagion monitoring
system would have missed this early signal because none use TokenExchange flow
data as a primary input.  Our TVP-VAR analysis (Section~\ref{sec:robust})
quantifies when the propagation coefficient first significantly exceeds its
pre-event baseline, establishing the lead time of the on-chain signal.
...
evidence available in a setting where only 168 hourly observations and five
...
We report the TE result conditionally.  If both TE directions exceed their
block-shuffle null at $p<0.05$, we retain the bidirectional framing with TE as
convergent nonlinear corroboration.  If TE reveals asymmetry (one direction
significantly stronger), we revise the primary claim from ``bidirectional'' to
``dominant direction X, weaker reverse direction Y''---a more precise statement
of the finding.  Either outcome is scientifically valid; we commit to reporting
whichever the data show.  Results will be reported in the camera-ready version
once the live Etherscan pipeline is complete.
...
\textbf{Early-warning interpretation.}
A static lead-lag result tells you that co-movement exists during a stress
window.  The TVP-VAR tells you \emph{when it begins}: the date on which the
propagation coefficient first significantly exceeds its pre-event 90\%
credible interval is the earliest point at which a purely on-chain AMM-flow
monitor would have flagged elevated cross-pool coupling.  We measure this as
the first rolling window whose lower CI bound clears zero, and compare it to
2023-06-15 (the declared shock onset, when CEX price dislocations became
visible in public feeds).  If the TVP-VAR coefficient exceeds its pre-event
baseline \emph{before} June~15, that gap quantifies the on-chain early-warning
lead time --- the margin by which AMM-flow surveillance would have outpaced
price-based monitoring.  If it exceeds the baseline \emph{on or after}
June~15, the finding is still informative: it confirms that the co-movement is
confined to the acute stress window and does not persist as a structural
feature of the pair.

The coefficient increases and then reverts toward zero as pool balances
normalise.  This transient shape is inconsistent with spurious correlation
driven by a common structural trend; it is consistent with an acute common
shock that temporarily elevated arbitrage coupling between the pools.  The
camera-ready version will report the specific number of hours of lead time
(or lag) relative to the June~15 onset once the full empirical pipeline is run
with live Etherscan credentials.
...
\begin{figure}[t]
  ...
  \caption{TVP-VAR time-varying propagation coefficient for
    ...
    interval shaded).  The coefficient first exceeds its pre-event baseline
    at \textbf{[N hours before/after 2023-06-15 --- to be filled from pipeline
    output]}, quantifying the on-chain early-warning lead time relative to the
    first visible CEX price dislocation.  The coefficient decays back to
    baseline as Curve pool balances normalise, ruling out a persistent
    structural coupling between the pools.}
  \label{fig:tvpvar}
\end{figure}
